...
public:
    ...
    void reset(float x0 = 0.0f, float y0 = 0.0f);

    /// Call every control-loop tick.
    /// @param vx_mm_s  current velocity along X axis (mm/s, positive = right)
    /// @param vy_mm_s  current velocity along Y axis (mm/s, positive = forward/up)
    /// @param dt_ms    elapsed time since last call (ms)
    void update(float vx_mm_s, float vy_mm_s, uint32_t dt_ms);
    ...
    bool getPositionAt(uint32_t timestamp_ms, float &out_x, float &out_y) const;
    ...
    void applyCorrection(float error_x, float error_y, uint32_t blend_ms = 200);

    /// Get the current best-estimate position (with any blended correction).
    void getCurrentPosition(float &out_x, float &out_y) const;

    /// Raw estimated position (without active correction blend)
    void getRawPosition(float &out_x, float &out_y) const;
